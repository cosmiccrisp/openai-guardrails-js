...
vi.mock('../../runtime', () => ({
    ...
    instantiateGuardrails: vi.fn(() => Promise.resolve([
        {
...
describe('GuardrailAgent', () => {
    ...
    describe('guardrail function creation', () => {
        ...
        it('should handle guardrail execution errors based on raiseGuardrailErrors setting', async () => {
            ...
            vi.mocked(instantiateGuardrails).mockImplementationOnce(() => Promise.resolve([
                {
                    definition: { name: 'Keywords' },
                    config: {},
            ...
            await expect(guardrailFunctionStrict.execute({ input: 'test' }))
                .rejects.toThrow('Guardrail execution failed');
        });
    });
});
