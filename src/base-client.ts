...
export abstract class GuardrailsBaseClient {
            ...
            if (Array.isArray(content)) {
                ...
                for (const part of content) {
                    if (typeof part === 'object' && part !== null) {
                        ...
                        if (['input_text', 'text', 'output_text', 'summary_text'].includes(partType) && typeof textVal === 'string') {
                            parts.push(textVal);
                        }
                    }
                }
     ...
     */
    protected async setupGuardrails(config: string | PipelineConfig, context?: GuardrailLLMContext): Promise<void> {
        ...
        if (typeof data === 'string') {
            ...
            if (typeof currentContent === 'string') {
                ...
                for (const part of currentContent) {
                    if (typeof part === 'object' && part !== null) {
                        const partType = part.type;
                        if (['input_text', 'text', 'output_text', 'summary_text'].includes(partType) && 'text' in part) {
    ...
    public async runStageGuardrails(
        ...
        try {
            ...
            if (!suppressTripwire && error instanceof Error && error.constructor.name === 'GuardrailTripwireTriggered') {
                throw error;
            }
            throw error;
        }
     ...
     */
    protected appendLlmResponseToConversation(conversationHistory: any[] | string | null, llmResponse: any): any[] {
        ...
        else if (llmResponse.choices && Array.isArray(llmResponse.choices) && llmResponse.choices.length > 0) {
