 ...
 */
function validateUrlSecurity(urlString: string, config: UrlsConfig): { parsedUrl: URL | null; reason: string } {
    try {
        let parsedUrl: URL;
    ...
    for (const allowedEntry of allowList) {
        ...
        if (entry.includes('://')) {
            try {
                ...
                console.warn(`Warning: Invalid URL in allow list: "${entry}" - ${error instanceof Error ? error.message : error}`);
            }
            continue;
        }
 ...
 */
export const urls: CheckFn<UrlsContext, string, UrlsConfig> = async (
    ctx,
    data,
    config
) => {
    ...
    for (const urlString of detectedUrls) {
        ...
        } else if (isUrlAllowed(parsedUrl, actualConfig.url_allow_list, actualConfig.allow_subdomains)) {
            allowed.push(urlString);
        } else {
            blocked.push(urlString);
