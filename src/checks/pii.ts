...
export const PIIConfig = z.object({
    ...
    block: z.boolean().default(false).describe("If true, triggers tripwire when PII is detected. If false, masks PII without blocking.")
});

export type PIIConfig = z.infer<typeof PIIConfig>;

// Schema for registry registration (without optional properties)
export const PIIConfigRequired = z.object({
...
const DEFAULT_PII_PATTERNS: Record<PIIEntity, RegExp> = {
    ...
    [PIIEntity.URL]: /\bhttps?:\/\/(?:[-\w.])+(?:\:[0-9]+)?(?:\/(?:[\w\/_.])*(?:\?(?:[\w&=%.])*)?(?:\#(?:[\w.])*)?)?/g,
...
function _asResult(
    ...
    const checkedText = piiFound
        ? _scrubPii(text, detection, config)
        : text;
