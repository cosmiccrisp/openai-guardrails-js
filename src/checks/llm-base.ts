...
export function buildFullPrompt(systemPrompt: string): string {
    ...
    if (systemPrompt.includes('JSON') || systemPrompt.includes('json') || systemPrompt.includes('{')) {
...
export async function runLLM(
    ...
    try {
        ...
        if (error instanceof SyntaxError || (error as any)?.constructor?.name === 'SyntaxError') {
            console.warn("LLM returned non-JSON or malformed JSON. Failing closed (flagged=true).", error);
            return {
                flagged: true,
                confidence: 1.0,
            } as LLMOutput;
