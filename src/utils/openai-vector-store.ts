...
async function getFilePaths(path: string): Promise<string[]> {
    ...
    try {
        ...
        throw new Error(`Error reading path ${path}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
async function uploadFiles(client: OpenAI, filePaths: string[]): Promise<string[]> {
    ...
    for (const filePath of filePaths) {
        try {
            ...
            console.warn(`Failed to upload file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
