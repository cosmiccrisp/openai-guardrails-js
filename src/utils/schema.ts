 ...
 */
export function resolveRef(schema: Record<string, unknown>, root: Record<string, unknown>): Record<string, unknown> {
    if (!isDict(schema)) {
    ...
    for (const [key, value] of Object.entries(schema)) {
        ...
        if (isDict(value)) {
            ...
            resolved[key] = value.map(item =>
                isDict(item) ? resolveRef(item, root) : item
            );
        } else {
            resolved[key] = value;
        }
 ...
 */
export function validateJson(jsonStr: string, schema: Record<string, unknown>, partial: boolean = false): unknown {
    try {
